...
public:
  PointCloudl::Ptr getLabeledCloud() const {
      return labeled_point_cloud_;
  }

  int getLabelId(const std::string& class_name) const {
      auto it = dict_.find(class_name);
      return (it != dict_.end()) ? it->second : 0;
  }
