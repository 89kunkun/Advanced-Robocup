...
class WaveCustomerDetect:
    ...
    def control_callback(self, msg):
        # Control detection activation/deactivation
    ...
    def skeleton_callback(self, msg):
        # Process skeleton message and detect waving gesture
        if not self.active:
            return
        for skeleton in msg.skeletons:
            # Convert keypoints to dict for easy access
            ...
            # Calculate height difference between shoulder and wrist (y increases downward)
            ...
            # Check if either wrist is higher than the corresponding shoulder (waving gesture)
            # wrist.y < shoulder.y means hand is raised
            if left_diff > self.waving_threshold or right_diff > self.waving_threshold:
                # Check cooldown to avoid frequent publishing
                ...
                # Calculate the center position of the waving person in image coordinates
                person_center_x = (left_shoulder.x + right_shoulder.x) / 2
                person_center_y = (left_shoulder.y + right_shoulder.y) / 2
                
                # Calculate bounding box: use all high-confidence keypoints if available, otherwise use shoulder positions with padding
                ...
                if len(all_x) > 0 and len(all_y) > 0:
                    ...
                    # Fallback: use only shoulder positions with padding
                ...
                # Determine waving person's position in image (left/center/right)
                ...
                # Publish person's image coordinates (Point message, z reserved)
                position_msg = Point()
                ...
                # Publish bounding box (RegionOfInterest message)
