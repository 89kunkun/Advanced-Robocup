...
def compute_axes_cereal(pts, vertical_cos_threshold):
    ...
    # (B) project mid/thin (置0投影)
    axis_mid  = normalize(project_to_yz(mid_raw))   # x=0
    axis_thin = normalize(project_to_xz(thin_raw))  # y=0

    # (C) swap mid and thin
    axis_mid, axis_thin = axis_thin, axis_mid

    # (D) build right-handed frame from red and green seed
    axis_green = axis_mid
    if np.linalg.norm(axis_green) < 1e-6:
    ...
    axis_blue = normalize(np.cross(axis_red, axis_green))
    if np.linalg.norm(axis_blue) < 1e-6:
        axis_blue = np.array([0.0, 0.0, 1.0])
    axis_blue = normalize(axis_blue)
    axis_green = normalize(np.cross(axis_blue, axis_red))

    # (E) FINAL output projection constraints + re-orthogonalize
    axis_red = normalize(project_to_xy(axis_red))  # z=0

    axis_green = normalize(project_to_yz(axis_green))  # x=0
    if np.linalg.norm(axis_green) < 1e-6:
        axis_green = np.array([0.0, 1.0, 0.0])
    axis_green = normalize(axis_green)

    axis_blue = normalize(np.cross(axis_red, axis_green))
    axis_blue = normalize(project_to_xz(axis_blue))  # y=0
    if np.linalg.norm(axis_blue) < 1e-6:
        axis_blue = np.array([0.0, 0.0, 1.0])
    axis_blue = normalize(axis_blue)

    axis_green = normalize(np.cross(axis_blue, axis_red))
    axis_green = normalize(project_to_yz(axis_green))
    if np.linalg.norm(axis_green) < 1e-6:
        axis_green = np.array([0.0, 1.0, 0.0])
    axis_green = normalize(axis_green)
