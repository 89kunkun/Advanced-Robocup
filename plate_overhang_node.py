...
class PlateOverhangNode:
    def __init__(self):
        ...
        self.nn_radius = float(rospy.get_param("~nn_radius", 0.01))   # 1cm
    ...
    # --------- RUN CHECK (same logic as before) ----------
    def _run_check_and_publish(self, msg: PointStamped):
        P = np.array([msg.point.x, msg.point.y, msg.point.z], dtype=np.float32)
        ...
        for k in range(self.max_steps + 1):
            ...
            if inside:
                ...
                rospy.loginfo(
                    f"[AUTO-CHECK] FOUND step={k}  P=({P[0]:.3f},{P[1]:.3f},{P[2]:.3f})  "
                    f"nearest={dmin:.4f} <= {self.nn_radius:.4f}  => TRUE (published /updated_grasp_point)"
                )
                return True

            self.pub_if.publish(Bool(data=False))
            rospy.loginfo(
                f"[AUTO-CHECK] step={k}  P=({P[0]:.3f},{P[1]:.3f},{P[2]:.3f})  "
        ...
        self.pub_if.publish(Bool(data=False))
        rospy.logwarn(
            f"[AUTO-CHECK] NOT FOUND within max_steps={self.max_steps}. "
            f"start=({msg.point.x:.3f},{msg.point.y:.3f},{msg.point.z:.3f}), step_x={self.step_x}"
        )
        return False
