...
# ==============================
# ✅ 新增：决定下一步抓物体还是抓盘子（最小改动）
# ==============================
class DecideNextGraspState(smach.State):
    def __init__(self):
        smach.State.__init__(self,
                             outcomes=['GRASP_NEXT_OBJECT', 'GRASP_PLATE'],
                             input_keys=['order_ids'])

    def execute(self, userdata):
        try:
            ids = userdata.order_ids if hasattr(userdata, 'order_ids') else []
        except Exception:
            ids = []
        if ids and len(ids) > 0:
            rospy.loginfo("[DecideNextGrasp] order_ids remaining: %s -> grasp next object", ids)
            return 'GRASP_NEXT_OBJECT'
        rospy.loginfo("[DecideNextGrasp] order_ids empty -> grasp plate")
        return 'GRASP_PLATE'
...
def main():
    ...
    with sm:
        ...
        rospy.loginfo("Adding NAV_TO_START2")
